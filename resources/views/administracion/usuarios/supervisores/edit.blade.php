...
<form action="{{ route('usuarios.supervisores-update',$supervisor) }}" method="POST">
    ...
    <div class="mb-5">
        ...
        <input type="text" id="name" name="name"
            ...
            placeholder="Nombre del nuevo supervisor" autocomplete="off" value="{{ $supervisor->name }}">
    ...
    <div class="mb-5">
        ...
        <select name="role_id" id="role" class="w-full p-4 rounded bg-gray-50">
            ...
            @foreach ($roles as $role)
                <option {{ ($role->id == $supervisor->role_id) ? 'selected' : ''}} value="{{ $role->id }}">{{ $role->name }}</option>
            @endforeach
