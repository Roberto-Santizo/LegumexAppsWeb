...
<div class="bg-white p-6 rounded-lg shadow-lg mt-5 container xl:w-2/3  mx-auto">
    <form action="{{ route('planSemanal.store') }}" method="POST" id="formulario6" novalidate>
        @csrf

        <x-alertas />

        <div class="mb-5">
            <label for="finca_id" class="mb-2 block uppercase text-gray-500 font-bold ">Elija la finca: </label>
            ...
            @error('finca_id')
            <p class="{{ $clasesAlerta }}">Seleccione un planta
            <p>
                @enderror
        </div>
