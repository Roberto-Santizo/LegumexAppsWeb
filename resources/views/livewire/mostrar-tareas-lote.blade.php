    ...
    <div class="flex flex-col md:grid md:grid-cols-3 md:grid-rows-4 mt-5 justify-between p-5 rounded-xl shadow-xl border-l-8 border-green-500 ">
            ...
            <div class="{{ $successMessage ? 'row-start-2' : 'row-start-1' }} col-span-2 row-span-3 md:text-base text-xs">
                ...
                @can('create plan semanal')
                    <p ><span class="uppercase font-bold text-black">Presupuesto:</span> <span class="text-green-500 font-black">Q{{ $tarea->presupuesto }}</span></p>
                @endcan
    

                @if($tarea->asignacion)
                    <p><span class="uppercase font-bold">Fecha de Asignación:</span> {{ $tarea->asignacion->created_at->format('d-m-Y h:i:s A') }}</p>
                @endif
    
                @if($tarea->cierre)
                    <p><span class="uppercase font-bold">Fecha de cierre: </span>{{$tarea->cierre->created_at->format('d-m-Y h:i:s A') }}</p>
                @endif
            </div>
