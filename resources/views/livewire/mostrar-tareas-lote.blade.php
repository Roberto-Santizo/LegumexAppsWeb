    ...
    @forelse ($tareas as $tarea)
        <div
            ...
            <div
                ...
                @if (!$tarea->asignacion)
                    <p><span class="uppercase font-bold">Cupos disponibles:</span>
                        {{ $tarea->personas - $tarea->cupos_utilizados }}</p>
                @else
                    <p><span class="uppercase font-bold">Total de personas asignadas:</span>
                        {{ $tarea->users->count() }}</p>
                @endif
                ...
                @if ($tarea->cierre)
                    <p><span class="uppercase font-bold">Fecha de cierre:
                        </span>{{ $tarea->cierre->created_at->format('d-m-Y
                                        h:i:s A') }}</p>
                @endif
            </div>
                            ...
                            <div class="mt-5 flex md:flex-col flex-row justify-center items-center gap-5">
                                @if ($tarea->cierreParcialActivo()->get()->isEmpty())
                                    <button wire:click="$dispatch('InsumosValidar',{{ $tarea->id }})">
                                        <i
                                            class="fa-solid fa-circle-check  text-2xl cursor-pointer hover:text-gray-500"></i>
                                    </button>
                                    ...
                                    <iconify-icon icon="zondicons:pause-solid"
                                        class="text-2xl cursor-pointer hover:text-gray-500 text-orange-500"
                                        title="Pausar"
                                        wire:click="$dispatch('pausarTarea',{{ $tarea->id }})"></iconify-icon>
                                @else
                                    <iconify-icon icon="gridicons:play"
                                        class="text-3xl cursor-pointer hover:text-gray-500 text-green-500"
                                        title="Reanudar"
...
@push('scripts')
    ...
    <script>
        Livewire.on('eliminar', tareaId => {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    Livewire.dispatch('eliminarTarea', {
                        tarea: tareaId
                    });
        ...
        Livewire.on('cleanTask', tareaId => {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    Livewire.dispatch('limpiarTarea', {
                        tarea: tareaId
                    });
        ...
        Livewire.on('InsumosValidar', tareaId => {
            Livewire.dispatch('validarInsumos', {
                tarea: tareaId
            });
        })

        Livewire.on('pausarTarea', tareaId => {
            Livewire.dispatch('pausar', {
                tarea: tareaId
            });
        })

        Livewire.on('reanudarTarea', tareaId => {
            Livewire.dispatch('reanudar', {
                tarea: tareaId
            });
        })
    </script>
