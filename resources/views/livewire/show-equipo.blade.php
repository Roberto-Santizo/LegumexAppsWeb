<div class="flex flex-col gap-10">
    ...
    <div class="mt-5 shadow-xl p-10 uppercase md:flex md:justify-between space-y-5">
        <div class="md:text-xl text-xs">
            ...
            <p><span class="font-bold">Fecha de adquisición:</span>
                {{ $equipo->acquisition_date ? $equipo->acquisition_date->format('d-m-Y') : 'sin datos' }}</p>
            <p><span class="font-bold">Fecha de instalación:</span>
                {{ $equipo->installation_date ? $equipo->installation_date->format('d-m-Y') : 'sin datos' }}</p>
            ...
            <button wire:click='Actualizar'
                class=" text-white p-2 font-bold  rounded w-1/2 mt-5 text-center {{ $equipo->status ? 'bg-green-500' : 'bg-red-500' }}">
                {{ $equipo->status ? 'ACTIVO' : 'DESACTIVADO' }}</button>
        </div>
        <div>
            <x-generate-q-r :url='$equipo->folder_url' />
    ...
    <div class="md:grid md:grid-cols-2 md:space-x-5 md:space-y-0 flex flex-col space-y-10 md:text-xl text-xs">
        <div>
            <h2 class="font-bold md:text-4xl">Trabajos Preventivos <i class="fa-solid fa-clock text-orange-500"></i>
            </h2>
            ...
            @foreach ($this->equipo->trabajosPreventivos as $trabajo)
                <div class="flex flex-col gap-5 mt-5">
                    <div class="p-5 shadow-xl flex justify-between">
                        ...
                        <p><span class="font-bold">Fecha de planificación:
                            </span>{{ $trabajo->fecha_planificacion->format('d-m-Y') }}</p>
        ...
        <div>
            <h2 class="font-bold md:text-4xl">Trabajos Realizados <i
                    class="fa-solid fa-circle-check text-green-500"></i></h2>
            ...
            @foreach ($this->equipo->trabajosRealizados as $trabajo)
                <div class="flex flex-col gap-5 mt-5">
                    <div class="p-5 shadow-xl flex justify-between">
                        <p><span class="font-bold">Tarea: </span>{{ $trabajo->descripcion }}</p>
                        <p><span class="font-bold">Fecha de cierre:
                            </span>{{ $trabajo->fecha_realizacion->format('d-m-Y') }}</p>
                    </div>
                </div>
            @endforeach
        </div>
