<div class="md:w-1/2 mx-auto">
    <form class="space-y-5" wire:submit.prevent='save'>
        ...
        <div class="flex flex-col">
            <label class="font-bold text-xl" for="imagenes">Área:</label>
        ...
        <div class="flex flex-col">
            <label class="font-bold text-xl" for="imagenes">Área:</label>
            <input wire:model='imagenes' type="file" class="w-full border p-5" multiple accept="image/jpeg, image/png">
            @error('imagenes')
                <x-alerta-error :message="$message" />
            @enderror
        </div>
        ...
        <fieldset>
            <table class="w-full mb-5 mt-5">
                <thead class="bg-sky-600">
                    <tr>
                        <th scope="col" class="text-white text-sm md:text-xl p-2">Accesorio</th>
                    ...
                    @foreach ($accesorios_all as $accesorio)
                        <tr class="odd:bg-orange-200 odd:text-white">
                            ...
                            <td class="text-center">
                                <input class="h-6 w-6 mt-2 lavadas" type="checkbox" wire:model="accesorios" value="{{ $accesorio->id }}">
                            </td>
                        </tr>
                    @endforeach
                </tbody>
        ...
        <div wire:loading>
            <div class="w-full flex justify-center items-center" style="height: 200px;">
                <div class="spinner-border animate-spin w-8 h-8 border-4 border-t-transparent rounded-full" role="status"></div>
            </div>
        </div>
