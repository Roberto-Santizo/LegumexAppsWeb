<div>
    <div class="flex fixed inset-0 z-40 items-center justify-center bg-gray-500 bg-opacity-75 md:text-base text-sm">
        <div class="h-2/3 md:w-1/3 w-2/3 flex flex-col gap-5 bg-white rounded shadow-xl p-5">
            <div class="flex justify-between items-center">
                <h2 class="p-2 font-bold md:text-xl text-sm uppercase">Recuento de Uso de Insumos</h2>
            ...
            <div class="space-y-5 overflow-y-auto">
                ...
                @foreach ($insumos as $insumo)
                    <div class="p-2 border bg-gray-100 shadow rounded-lg hover:bg-gray-200 cursor-pointer"
                        wire:click='openModalCantidad({{ $insumo }})'>
                        {{ $insumo->insumo }}
                    </div>
                @endforeach
            </div>
    ...
    @if ($open)
        <livewire:add-insumo-cantidad-modal :insumo="$selectedInsumo" :editing="false"/>
    @endif
</div>
