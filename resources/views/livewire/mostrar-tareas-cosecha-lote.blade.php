<div>
    <div class="mt-5 flex flex-col md:flex-row justify-between p-5 rounded-xl shadow-xl border-l-8 border-green-500 ">
        <div class="text-xs md:text-xl">
            ...
            <div
                class="border border-green-500 bg-green-100 text-green-700 font-bold uppercase p-2 mb-5 text-sm flex flex-row gap-2 items-center">
        ...
        <div class="flex flex-col items-center justify-between">
            <div class="flex flex-col gap-5">
                ...
                @if (!$asignacionSinCierre && !$tarea->cierreSemanal && !$asignacionSinLibras)
                <button wire:click="$dispatch('terminar',{{ $tarea->id }})">
                    <i title="Finalizar Cosecha Semanal"
                        class="fa-solid fa-calendar-check  text-2xl cursor-pointer hover:text-gray-500"></i>
                </button>
                @endif

                @else
                @if ($tarea->asignaciones->count() == 0)
                @can('create plan semanal')
