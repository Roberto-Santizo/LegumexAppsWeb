...
(function(){
    const formulario = document.getElementById('formularioP3');
    const canvasPads = [];
    const signaturePads = [];
    ...
    async function guardarFirmas() {
        ...
        for (let i = 29; i < 41; i++) {
            const canvas = canvasPads[i - 29]; // Ajustamos el índice
            const firmaFile = await new Promise((resolve) => canvas.toBlob(resolve, 'image/png'));
            datos.append(`firma${i - 28}`, firmaFile, `firma${i - 28}.png`); // Ajustamos el índice al guardar
        }
            ...
            if (resultado.status) {
                for (let i = 29; i <= 41; i++) {
                    ...
                    if (firmaInput) {
                        ...
                        firmaInput.value = resultado.imagenes[`firma${i - 28}`];
                    }
                }
