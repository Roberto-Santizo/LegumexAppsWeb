...
class DocumentoCPController extends Controller
{
    ...
    public function document(Documentocp $documentocp)
    {
        ...
        $areasWithElementCount = $areas->map(function ($area) {
            return [
                'area' => $area,
                'element_count' => $area->elementos->count()
            ];
        });

        $areasWithElementCount = $areasWithElementCount->sortByDesc('element_count');
        ...
        foreach ($areasWithElementCount as $areaData) {
            if ($total1 <= $total2) {
                $chunk1->push($areaData['area']);
                $total1 += $areaData['element_count'];
            } else {
                $chunk2->push($areaData['area']);
                $total2 += $areaData['element_count'];
            }
        }

        return view('mantenimiento.documentoCP.document', [
            'documentocp' => $documentocp,
            'areasT1' => $chunk1,
            'areasT2' => $chunk2
        ]);
    }
